...
public class YniPickaxe extends ItemToolPickaxe implements YniGear {
    private final MaterialSet set;
    private final YniNameCache nameCache = new YniNameCache();

    public YniPickaxe(MaterialSet set, String texKey, int id, ToolMaterial material) {
        super(set.id + "_pickaxe", texKey, id, material);
        this.set = set;
    }
    ...
    public String getTranslatedName(ItemStack stack) {
        return nameCache.get(set, "Pickaxe");
    }

    @Override
    public String getTranslatedDescription(ItemStack stack) {
        return YniLore.describe(set, "Pickaxe", true);
    }
}
